...
async function batchAddListings() {
  ...
  for (let i = 0; i < cityListings.length; i++) {
    ...
    try {
      const response = await httpPost(listing);
      
      if (response.success) {
        console.log(`✓ [${i + 1}/${cityListings.length}] 成功添加: ${listing.city} - ${listing.title}`);
        successCount++;
      } else {
        console.log(`✗ [${i + 1}/${cityListings.length}] 添加失败: ${listing.city} - ${response.error}`);
        failCount++;
      }
    } catch (error) {
...
console.log('node batch_add_city_listings_simple.js');
