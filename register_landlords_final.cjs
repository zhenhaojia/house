    ...
    try {
        ...
        for (const landlord of landlords) {
            ...
            if (existingUsers.length > 0) {
                console.log(`⚠️ 用户 ${landlord.username} 已存在，跳过注册`);
                continue;
            }
        ...
        // 验证注册结果
        const [allLandlords] = await connection.execute(
            'SELECT id, username, email, phone, role FROM users WHERE role = ?',
            ['landlord']
        );
        
        console.log('\n🔍 数据库中的房东账号：');
        allLandlords.forEach((landlord, index) => {
            console.log(`${index + 1}. ${landlord.username} - ${landlord.email} - ${landlord.phone}`);
        });
